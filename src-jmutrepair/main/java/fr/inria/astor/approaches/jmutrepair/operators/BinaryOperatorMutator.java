...
public abstract class BinaryOperatorMutator extends SpoonMutator<CtExpression> {
	...
	protected void addRemainingsAndFoward(List<MutantCtElement> result, CtBinaryOperator<?> op,
			List<BinaryOperatorKind> operators) {
			...
			if (binaryOperatorKind.equals(op.getKind())) {
				...
				if (parentMutator != null) {
					...
					for (MutantCtElement left_i : mutationLeft) {
						CtBinaryOperator bop = createBinaryOp(op, binaryOperatorKind,
								(CtExpression) left_i.getElement(), rightOriginal);
						left_i.setElement(bop);
						result.add(left_i);
					}
					for (MutantCtElement right_i : mutationRight) {
						CtBinaryOperator bop = createBinaryOp(op, binaryOperatorKind, leftOriginal,
								(CtExpression) right_i.getElement());
				...
				// diff
				CtExpression right_c = factory.Core().clone(op.getRightHandOperand());
				CtExpression left_c = factory.Core().clone(op.getLeftHandOperand());

...
public class IngredientSpaceWithCTElementsTest {
	...
	public void testClassScope() throws JSAPException {
		...
		// modif point 0 and 1 are from the same class, so they must be mapped
		// to the same key
		assertEquals(key0, key1);

		// modif point 0 and 8 are from different classes, so they must be
		// mapped to the different keys
		assertNotEquals(key0, key8);

		List<?> ingredientsfromK0 = locIngSpace.getIngredients(point1);
		List<?> ingredientsfromK8 = locIngSpace.getIngredients(point8);
	...
	public void testPackageScope() throws JSAPException {
		...
		List<?> ingredientsfromK0 = packageScope.getIngredients(point0);
		List<?> ingredientsfromK8 = packageScope.getIngredients(point8);
	...
	public void testGlobalScope() throws JSAPException {
		...
		List<?> ingredientsfromK0 = globalSpace.getIngredients(point0);
		List<?> ingredientsfromK8 = globalSpace.getIngredients(point8);
	...
	public void testLocalComplete() throws Exception {
		...
		assertTrue(susp.getQualifiedName().equals("org.apache.commons.math.analysis.solvers.BisectionSolver") || susp
				.getQualifiedName().equals("org.apache.commons.math.analysis.solvers.UnivariateRealSolverUtils"));
	}
